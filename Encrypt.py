...
class Image_Encryption(object):
    def __init__(self, n, t, k, img, plot_histogram=True, show_image=True, self_debug=True):
        ...
        info = open("info.txt", "w")  # open the info.txt file in write mode.
        img_info_file = open("img_info.txt", "w")  # open the img-info.txt file in write mode.
        ...
        for i in range(len(content_)):
            ...
            for j in range(0, content_[i][1], k):
                ...
                for p in range(iterator):
                    new_img_info[content_[i][0]] += [
                        (
                            x_reconstructed_polynomial.eval(e[p]),
                            y_reconstructed_polynomial.eval(e[p]),
                        )
                    ]
                count_x += 1
                count_y += 1
