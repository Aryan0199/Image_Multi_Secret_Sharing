...
class Scheme(object):
    ...
    def __init__(self, s, n, k, p):
        ...
        self.coefs = list(dict.fromkeys(
            [randint(1, 65000) for i in range(1, k)]))

    def construct_shares(self):
        self.coefs.append(self.s)
        values = np.polyval(
            self.coefs, [i for i in range(1, self.n + 1)]) % self.p
    ...
    def reconstruct_secret(shares: dict, inputs: list, k, p):
        ...
        for i in range(len(inputs)):
            indeksi.append(
                int(list(shares.keys())[list(shares.values()).index(inputs[i])]))
